...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Meu Criador👉👈\n' // Seu nome
            + 'ORG:Shaka Orion;\n' // Nome do bot
            + 'TEL;type=CELL;type=VOICE;waid=557591294651:+55 75 9128-4651\n' //Seu número Whatsapp
            + 'END:VCARD' ;
...
conn.on('open', () => {
	app.get('/', (request, response) => {
	 conn.updatePresence(Presence.availabe); 
	...
	conn.sendMessage(
		'557591294651@s.whatsapp.net',
		'🤖 ƁѲƭ  ☄️✨𝑺𝒉𝒂𝒌𝒂 𝑶⃟𝑹𝑰𝑶𝑵✨☄️  ØŇŁƗŇ€ ✅',
		MessageType.text
	);
});
...
client.on('CB:action,,battery', json => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [`$557591294651@s.whatsapp.net`] // substitua isso pelo seu número
            const mod = [ownerNumber,"$557591294651@s.whatsapp.net"]//mude o seu numero
            const adminbotnumber = ["$557591294651@s.whatsapp.net"]//mude o seu numero
			const frendsowner = ["$557591294651@s.whatsapp.net"]//mude o seu numero
            const premium = JSON.parse(fs.readFileSync('./database/vip.json'))
            //const premium = ["557591294651@s.whatsapp.net","${setting.vip2}@s.whatsapp.net","${setting.vip3}@s.whatsapp.net","${setting.vip4}@s.whatsapp.net","${setting.vip5}@s.whatsapp.net","${setting.vip6}@s.whatsapp.net","${setting.vip7}@s.whatsapp.net","${setting.vip8}@s.whatsapp.net","${setting.vip9}@s.whatsapp.net","${setting.vip10}@s.whatsapp.net",]
...
vip = `✅@${mentioned[0].split('@')[0]}você virou Vip no Shaka Orion✅`
...
case 'antifake':
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* JHON\n*WPP:* wa.me/+557591294651\n*INSTA:* @_jhon.san\n\n\nEspero que tenham gostado do bot 🥵'})
					  client.sendMessage(from, 'Ctt do meu dono ai Wa.me/557591294651 , pfv n flode',MessageType.text, { quoted: mek} )
                    break
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: '[𝐓𝐫𝐚𝐧𝐬𝐦𝐢𝐬𝐬𝐚𝐨 𝐒𝐡𝐚𝐤𝐚]\n${body.slice(4)}'})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐓𝐫𝐚𝐧𝐬𝐦𝐢𝐬𝐬𝐚𝐨 𝐒𝐡𝐚𝐤𝐚 ]\n\n${body.slice(4)}`)
						}
...
case 'aviso':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐀𝐯𝐢𝐬𝐨 𝐒𝐇𝐀𝐊𝐀 𝐎𝐑𝐈𝐎𝐍]\n\n${body.slice(7)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
